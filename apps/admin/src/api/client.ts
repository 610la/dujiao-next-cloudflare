...
const isComplianceRequiredMessage = (message: string) =>
  message === 'compliance_required' || message === 'compliance_required_by_super_admin'
...
async function request(method: string, path: string, bodyOrOptions?: any, options?: RequestOptions): Promise<{ data: any; headers?: Record<string, string> }> {
  ...
  if (opts.blob) {
    ...
    if (!response.ok) {
      ...
      if (isComplianceRequiredMessage(message)) {
        return Promise.reject(createNotifiedError(message))
      }
      notifyError(message)
      return Promise.reject(createNotifiedError(message))
