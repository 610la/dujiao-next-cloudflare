...
export function useCheckout() {
  ...
  const manualFieldTypes = new Set(['text', 'textarea', 'phone', 'email', 'number', 'select', 'radio', 'checkbox', 'china_region'])
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  const phonePattern = /^\+?[0-9\-()\s]{6,20}$/
  const parseChinaRegionValue = (value: unknown) => {
    const [provinceCode = '', cityCode = '', districtCode = '', ...textParts] = String(value ?? '').trim().split('|')
    return {
      provinceCode,
      cityCode,
      districtCode,
      ...
      product.fields.forEach((field) => {
        ...
        if (field.type === 'checkbox') {
          formValues[field.key] = Array.isArray(currentValue)
            ...
            : []
        } else if (field.type === 'china_region') {
    ...
    manualFormProducts.value.forEach((product) => {
      ...
      product.fields.forEach((field) => {
        ...
        if (field.type === 'china_region') {
          const region = parseChinaRegionValue(rawValue)
          if (field.required && (!region.provinceCode || !region.cityCode || !region.districtCode || !region.text)) {
            setError(product.itemKey, field, t('checkout.manualFormFieldRequired', { name: fieldLabel }))
          }
          return
        }
    ...
    manualFormProducts.value.forEach((product) => {
      ...
      product.fields.forEach((field) => {
        ...
        if (field.type === 'china_region') {
          const region = parseChinaRegionValue(rawValue)
          if (region.text) {
            row[field.key] = String(rawValue).trim()
          }
          return
        }
        const text = rawValue == null ? '' : String(rawValue).trim()
  ...
  const recoverSubmittedCheckout = async (requestId: string) => {
    const delays = [300, 700, 1200]
    for (const delay of delays) {
      await new Promise((resolve) => setTimeout(resolve, delay))
      try {
        const response = userAuthStore.isAuthenticated
          ? await userOrderAPI.checkoutResult(requestId)
          : await guestOrderAPI.checkoutResult(requestId, {
              email: guestEmail.value.trim(),
              order_password: guestPassword.value,
            })
        const data = response.data.data
        if (data?.order_no) return data
      } catch {}
    }
    return null
  }
    ...
    try {
      ...
      const checkoutRequestId = await getOrCreateCheckoutRequestId({
        buyer: userAuthStore.isAuthenticated
          ? `user:${String(userAuthStore.user?.id || userAuthStore.user?.email || '')}`
          : `guest:${guestEmail.value.trim().toLowerCase()}`,
        order: basePayload,
      })
      const payload = {
        ...basePayload,
      ...
      try {
        ...
        if (!['NETWORK_ERROR', 'REQUEST_TIMEOUT'].includes(String(submitError?.code || ''))) throw submitError
        responseData = await recoverSubmittedCheckout(checkoutRequestId)
        if (!responseData) throw submitError
      }
      ...
      payQuery.set('order_no', String(responseData.order_no))
      if (!userAuthStore.isAuthenticated) {
        payQuery.set('guest', '1')
      }
      const paymentChannelId = responseData.channel_id || payload.channel_id
