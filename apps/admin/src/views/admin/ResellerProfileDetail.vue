...
const userDetailLink = computed(() => (profile.value?.user_id ? adminHref(`/users/${profile.value.user_id}`) : ''))
const siteConfigLink = computed(() => adminHref(`/resellers/site-configs?reseller_id=${profileId.value}`))
const productSettingsLink = computed(() => adminHref(`/resellers/product-settings?reseller_id=${profileId.value}`))
const ledgerLink = computed(() => adminHref(`/resellers/ledger-entries?reseller_id=${profileId.value}`))
const withdrawLink = computed(() => adminHref(`/resellers/withdraws?reseller_id=${profileId.value}`))
