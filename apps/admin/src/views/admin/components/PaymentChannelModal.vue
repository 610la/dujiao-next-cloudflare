  ...
  async (id) => {
    ...
    if (id === null) {
      ...
      try {
        ...
        configuredSecrets.value = Array.isArray(channel.configured_secrets) ? channel.configured_secrets : []
    ...
    <DialogScrollContent class="w-[calc(100vw-1rem)] max-w-3xl p-4 sm:p-6" @interact-outside="(e) => e.preventDefault()">
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div>
          ...
          <div v-if="configuredSecrets.length" class="mb-2 rounded-md border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-700 dark:text-emerald-300">
            {{ t('admin.paymentChannels.modal.savedSecretsHint', { count: configuredSecrets.length }) }}
          </div>
