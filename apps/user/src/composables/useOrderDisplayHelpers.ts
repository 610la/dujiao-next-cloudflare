...
import { formatSiteDateTime } from '../utils/time'
...
export function useOrderDisplayHelpers(order: Ref<any>) {
  ...
  const sanitizeInstructionsHtml = (raw: string) => processHtmlForDisplay(raw)
  ...
  const splitNonEmptyLines = (value: unknown) => {
    return String(value ?? '')
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean)
  ...
  const looksLikeAutoSecretLabel = (value: unknown) => {
  ...
  const cleanAutoDeliveryValue = (value: unknown) => {
    const text = String(value ?? '').trim()
    if (!text) return ''
  ...
  const autoSecretLinesFromPayload = (value: unknown) => {
  ...
  const fulfillmentDeliveryLines = (fulfillment: any) => {
    ...
    const lines: string[] = []
    const isAutoFulfillment = String(fulfillment?.type || '').trim() === 'auto'
    if (deliveryData && typeof deliveryData === 'object') {
      if (isAutoFulfillment && Array.isArray(deliveryData.card_secrets)) {
        const secrets = deliveryData.card_secrets
          .map((row: any) => String(row?.secret || '').trim())
          .filter(Boolean)
      ...
      entries.forEach((entry: any) => {
        ...
        if (isAutoFulfillment) {
          const normalized = cleanAutoDeliveryValue(value || key)
          if (normalized) {
    ...
    if (isAutoFulfillment && !lines.length) {
      return autoSecretLinesFromPayload(fulfillment?.payload)
    }
    return lines
  }
