...
import { SITE_TIME_ZONE, formatMoney, formatShortDate, getLocalizedText, parseSiteDate, toRFC3339 } from '@/utils/format'
...
const rankingTitle = (item: DashboardProductRanking) => getLocalizedText(item.title) || `#${item.product_id}`
...
const shortRangeEndDate = (value?: string) => {
  const date = parseSiteDate(value)
  if (!date) return formatShortDate(value)
  return formatShortDate(new Date(Math.max(0, date.getTime() - 1000)).toISOString())
}

const makeRangeDate = (raw: string, endOfDay: boolean) => {
  if (!raw) return undefined
  const suffix = endOfDay ? 'T23:59:59' : 'T00:00:00'
  return toRFC3339(`${raw}${suffix}`)
}
  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
      ...
      <div class="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
        <div class="w-full sm:w-[150px]">
          <Select v-model="filters.range" @update:model-value="handleRangeChange">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 [&>*]:min-w-0">
      ...
      <Card class="min-w-0">
        ...
        <CardContent>
          <div class="text-sm font-medium">{{ shortDate(overview?.from) }} - {{ shortRangeEndDate(overview?.to) }}</div>
        ...
        <CardContent>
          ...
          <div v-else class="space-y-2">
            <div v-for="item in rankings.top_products" :key="`${item.product_id}-${item.sku_id || 0}`" class="min-w-0 rounded-lg border border-border px-3 py-2">
              <div class="line-clamp-1 min-w-0 text-sm font-medium">{{ rankingTitle(item) }}</div>
