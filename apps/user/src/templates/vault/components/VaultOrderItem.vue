  ...
  <div class="flex flex-wrap justify-between gap-3.5 border-b py-3.5 first:pt-0 last:border-b-0 last:pb-0">
    <div class="flex min-w-0 flex-1 gap-3">
      ...
      <div class="min-w-0 flex-1">
        ...
        <div v-if="manualSubmissionRows(item.manual_form_submission, item.manual_form_schema_snapshot).length" class="mt-2.5 max-h-80 min-w-0 max-w-full overflow-x-hidden overflow-y-auto rounded-sm border bg-secondary px-3 py-2.5 text-[12.5px] text-muted-foreground">
          <div class="mb-1.5 font-bold text-muted-foreground">{{ t('orderDetail.manualSubmissionTitle') }}</div>
          <div v-for="row in manualSubmissionRows(item.manual_form_submission, item.manual_form_schema_snapshot)" :key="row.key" class="mb-0.5 min-w-0 break-words last:mb-0">
            <span class="font-semibold text-foreground">{{ row.label }}</span>：<span class="break-all whitespace-pre-wrap">{{ row.value }}</span>
          </div>
        </div>
      </div>
    </div>
