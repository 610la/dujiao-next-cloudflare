<template>
  <div class="grid gap-2">
    ...
    <template v-if="isFulfillmentTruncated(fulfillment)">
      ...
      <div class="mt-1">
        <div class="mb-3 text-[15px] font-bold text-foreground">发货内容:</div>
        <pre class="max-h-[260px] overflow-y-auto whitespace-pre-wrap break-all rounded-sm border bg-secondary p-3 font-mono text-[12.5px] text-muted-foreground">{{ fulfillment.payload }}</pre>
      </div>
    </template>
    <div v-else-if="fulfillmentDeliveryLines(fulfillment).length" class="mt-1">
      <div class="mb-3 text-[15px] font-bold text-foreground">发货内容:</div>
      <div class="grid gap-0.5 rounded-sm border bg-secondary p-3 font-mono text-[12.5px] text-muted-foreground">
        <div v-for="(line, index) in fulfillmentDeliveryLines(fulfillment)" :key="index" class="whitespace-pre-wrap break-all">{{ line }}</div>
      </div>
    </div>
    <div v-else class="mt-1">
      <div class="mb-3 text-[15px] font-bold text-foreground">发货内容:</div>
      <pre class="whitespace-pre-wrap break-all rounded-sm border bg-secondary p-3 font-mono text-[12.5px] text-muted-foreground">{{ fulfillment.payload }}</pre>
    </div>
