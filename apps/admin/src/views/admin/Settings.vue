...
const fetchSettings = async () => {
  ...
  try {
    ...
    if (smtpRes.data && smtpRes.data.data) {
      ...
      smtpData.transport = smtp.transport === 'resend' ? 'resend' : 'cloudflare'
      ...
      const cloudflare = smtp.cloudflare as Record<string, unknown> | undefined
      smtpData.cloudflare.account_id = String(cloudflare?.account_id || '')
      smtpData.cloudflare.zone_id = String(cloudflare?.zone_id || '')
      smtpData.cloudflare.api_token = ''
      smtpData.cloudflare.has_api_token = !!cloudflare?.has_api_token
      const resend = smtp.resend as Record<string, unknown> | undefined
      smtpData.resend.api_key = ''
      smtpData.resend.has_api_key = !!resend?.has_api_key
