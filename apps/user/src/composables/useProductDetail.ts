...
import { formatSiteDate } from '../utils/time'
...
export function useProductDetail(options: { onLoaded?: () => void } = {}) {
  ...
  const formatRelatedPostDate = (dateString: string) => {
    return formatSiteDate(dateString, appStore.locale, { year: 'numeric', month: 'long', day: 'numeric' })
  }
  ...
  const productSeoMeta = computed(() => product.value?.seo_meta || {})
  const productSeoKeywords = computed(() => {
    const keywords = productSeoMeta.value.keywords
    return getLocalizedText(keywords) || (typeof keywords === 'string' ? keywords : '')
  })
  const productSeoDescription = computed(() => {
    const description = productSeoMeta.value.description
    return getLocalizedText(description) || (typeof description === 'string' ? description : '')
  })

  usePageSeo({
    canonicalPath: () => product.value?.slug ? `/products/${product.value.slug}` : route.path,
  ...
  useHead({
    meta: () => {
      ...
      if (productSeoKeywords.value) tags.push({ name: 'keywords', content: productSeoKeywords.value })
