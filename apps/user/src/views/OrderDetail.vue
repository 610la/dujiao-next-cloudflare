              ...
              <div class="flex min-w-0 flex-1 items-start gap-3 overflow-hidden">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div v-if="manualSubmissionRows(item.manual_form_submission, item.manual_form_schema_snapshot).length"
                    class="mt-3 max-h-80 min-w-0 max-w-full overflow-x-hidden overflow-y-auto rounded-xl border border-gray-200 bg-white p-3 text-xs text-gray-600 dark:border-white/10 dark:bg-black/30 dark:text-gray-300">
                    <div class="mb-2 font-semibold text-muted-foreground">{{ t('orderDetail.manualSubmissionTitle') }}</div>
                    <div v-for="row in manualSubmissionRows(item.manual_form_submission, item.manual_form_schema_snapshot)" :key="row.key" class="mb-1 min-w-0 break-words last:mb-0">
                      <span class="text-foreground">{{ row.label }}</span>：<span class="break-all whitespace-pre-wrap">{{ row.value }}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="mt-4">
                ...
                <div v-if="child.fulfillment">
                  ...
                  <div v-if="isFulfillmentTruncated(child.fulfillment)" class="mt-3">
                    ...
                    <div>
                      <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
                      <div class="border rounded-xl p-4 overflow-hidden">
                      <div class="max-h-48 overflow-y-auto whitespace-pre-wrap break-all text-sm text-muted-foreground">{{ child.fulfillment.payload }}</div>
                      </div>
                    </div>
                  </div>
                  <div v-else-if="fulfillmentDeliveryLines(child.fulfillment).length"
                    class="mt-3 break-all overflow-hidden">
                    <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
                    <div class="space-y-1 rounded-xl border p-4 text-sm text-muted-foreground">
                  ...
                  <div v-else-if="child.fulfillment.payload"
                    ...
                    <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
                    <div class="rounded-xl border p-4 whitespace-pre-wrap break-all text-sm text-muted-foreground">{{ child.fulfillment.payload }}</div>
                  </div>
        ...
        <div v-if="order.fulfillment"
          ...
          <div v-if="isFulfillmentTruncated(order.fulfillment)" class="mt-4">
            ...
            <div>
              <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
              <div class="border rounded-xl p-4 overflow-hidden">
              <div class="max-h-64 overflow-y-auto whitespace-pre-wrap break-all text-sm text-muted-foreground">{{ order.fulfillment.payload }}</div>
              </div>
            </div>
          </div>
          <div v-else-if="fulfillmentDeliveryLines(order.fulfillment).length"
            class="mt-4 break-all overflow-hidden">
            <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
            <div class="space-y-1 rounded-xl border p-4 text-sm text-muted-foreground">
          ...
          <div v-else
            ...
            <div class="mb-3 text-base font-bold text-foreground">发货内容:</div>
            <div class="rounded-xl border p-4 whitespace-pre-wrap break-all text-sm text-muted-foreground">{{ order.fulfillment.payload }}</div>
          </div>
