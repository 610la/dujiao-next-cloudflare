...
import { formatSiteDateTime, parseSiteDateTime } from '../utils/time'
import {
...
export function usePayment() {
  ...
  const autoPaymentStarted = ref(false)
  ...
  const expiresAtMs = computed(() => {
    ...
    if (!raw) return null
    return parseSiteDateTime(raw)?.getTime() ?? null
  })

  const orderExpired = computed(() => {
    if (!order.value?.expires_at) return false
    const ts = parseSiteDateTime(order.value.expires_at)?.getTime()
    if (!ts) return false
  ...
  watch(
    ...
    async ([, orderNo], [, previousOrderNo]) => {
      ...
      autoPaymentStarted.value = false
      if (routeChannelId.value) {
        selectedChannelId.value = routeChannelId.value
      }
      if (!orderNo) return
  ...
  watch(
    ...
    () => {
      if (!selectedChannelId.value && requiresOnlineChannel.value && channels.value.length === 1) {
        const onlyChannel = channels.value[0]
        if (onlyChannel && !isChannelDisabledForAmount(onlyChannel)) {
          selectedChannelId.value = Number(onlyChannel.id)
        }
      }
  ...
  watch(
    () => [
      autoPayRequested.value,
      order.value?.status,
      selectedChannelId.value,
      paymentResult.value?.payment_id,
      paymentResult.value?.pay_url,
      paymentResult.value?.qr_code,
      configReady.value,
      showGuestAuthForm.value,
      canSubmitPayment.value,
    ],
    () => {
      if (!autoPayRequested.value || autoPaymentStarted.value) return
      if (!order.value || order.value.status !== 'pending_payment') return
