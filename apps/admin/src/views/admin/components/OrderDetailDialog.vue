...
import type { AdminOrder, AdminOrderItem, AdminProcurementOrder, AdminPayment } from '@/api/types'
...
import { adminHref } from '@/utils/adminHref'
...
const couponCodeLink = (code: string) => adminHref(`/coupons?code=${encodeURIComponent(code)}`)
...
const orderPaymentMethodLabel = (order: AdminOrder) => {
  ...
  if (payments.some((payment: any) => payment?.provider_type === 'manual' || payment?.channel_type === 'manual')) {
    return t('admin.orders.paymentMethodManual')
  }
  if (!hasWalletPayment(order) && !hasOnlinePayment(order) && ['paid', 'fulfilling', 'delivered', 'completed'].includes(order.status)) {
    return t('admin.orders.paymentMethodManual')
  }
              ...
              <CardContent class="p-3">
                ...
                <div class="text-sm text-foreground break-words">
                  <span v-if="selectedOrder.user_id">
                    ...
                    <a :href="userDetailLink(selectedOrder.user_id)" class="text-primary underline-offset-4 hover:underline">
                      {{ selectedOrder.user_display_name || selectedOrder.user_email || `#${selectedOrder.user_id}` }}
                    </a>
                    <span v-if="selectedOrder.user_email" class="ml-1 break-all text-muted-foreground">
